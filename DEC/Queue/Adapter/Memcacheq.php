 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class DEC_Queue_Adapter_Memcacheq extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function isExists($name)
    {
        ...
        foreach ($this->_queues as $queue) {
            ...
            $pants = explode(' ', $queue);
            $count = array();
            // cheat and set a count!
            if (isset($pants[1])) {
                $count = explode('/', $pants[1]);
            }
            if (isset($count[0])) {
            ...
            if (isset($pants[0]) && strstr($pants[0], $name)) {
                return true;
            }            
        }
        return false;
    ...
    public function send($message, Zend_Queue $queue=null)
    {
        ...
        $result = $this->_cache->set($queue->getName(), serialize($message), 0, 0);
        if ($result === false) {
            require_once 'Zend/Queue/Exception.php';
        ...
        if ($maxMessages > 0 ) {
            for ($i = 0; $i < $maxMessages; $i++) {
                $data = array(
                    ...
                    'body'   => unserialize($this->_cache->get($queue->getName())),
                );
                $msgs[] = $data;
            }
        }
